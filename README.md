...
> v1.1.0 新增。**全部在 App 内完成，不需要终端 / Xcode / 重新编译。**

1. **登记说话人**：设置 → 说话人识别 → 管理 → 输入名字 → 录 3 个样本（点麦克风开始 → 说「我是XXX，这是我的声音样本」→ 再点停止）。每个人都要录，至少 3 个样本。
2. **训练模型**：登记页点「训练模型 / 重新训练模型」→ 约 30 秒 → 模型就绪（只录了一个人时，App 会自动补一个背景噪音类别，保证能训练）。
3. **自动识别**：之后每次语音留言，App 自动识别是谁在说话：
   - 消息气泡底部显示说话人名字（如 Ethan）
   - 该消息自动进入**说话人专属会话**（如「Ethan 的聊天」）——AI 只看到这个人的聊天记录，**互不泄露**
