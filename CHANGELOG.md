...
## v0.7 — 2026-08-13：「小麦小麦」唤醒词

### 新增
- **免手唤醒**：输入栏耳朵按钮开启后持续监听，说「小麦小麦」（兼容同音字「小卖」）唤醒
- **唤醒应答**：命中后电脑用与 AI 回复完全相同的 Xiaoxiao 语音回答「我在！」
  - 麦克风全程不断——紧跟应答说话不丢字（唤醒 → 应答 → 连续 STT → AI 回复朗读）
- **唤醒词绝不算 STT 内容**：开麦初期 3.5 秒自动剥离开头噪音（唤醒词残余 + 应答「我在」），纯噪音不会变成消息
- 留言结束（静音 2 秒自动发送 / AI 回复朗读完）后自动重新挂起唤醒，免手续听
- 新增 `POST /api/speak`（App 端 `speak(_:)`）：朗读任意文本，播完才返回

### 界面
- 耳朵切换按钮：灰 = 关，橙 = 开
- 状态横幅区分：绿耳「在听「小麦小麦」…」（待命） / 红麦「在听… 静音 2 秒自动发送」（留言中）
- 待命横幅提供「关闭唤醒」按钮

### 修复
- **唤醒后 STT 接不上**（跟随「我在！」说话被忽略）：旧唤醒会话的迟到 recognition
  回调会再触发一次 stop()，把刚开的留言会话立即销毁并重新挂起唤醒。
  修复：会话代数（epoch）隔离——每次会话代数 +1，回调闭包捕获代数，
  不匹配当前代数（旧会话）一律忽略（`c7d7afd`）
- 唤醒命中后旧会话回调抢先自动重挂（与手动接管竞态）→ `rearmToken` 作废陈旧重挂
